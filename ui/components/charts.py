...
def equity_curve_chart(equity_curve: pd.DataFrame, benchmark: pd.Series | None = None) -> go.Figure:
...
def returns_distribution(returns: pd.Series) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(x=x, y=y, mode="lines", name="Normal Fit", line={"color": "orange", "width": 2})
    )
    fig.update_layout(
        height=350, template="plotly_dark", margin={"l": 0, "r": 0, "t": 30, "b": 0}, bargap=0.05
    )
    return fig
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker={
                    ...
                    "size": 20,
                    "color": "gold",
                    "line": {"width": 2, "color": "darkgoldenrod"},
                },
            )
        )
    fig.update_layout(
...
def correlation_heatmap_chart(corr: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(height=400, template="plotly_dark", margin={"l": 0, "r": 0, "t": 30, "b": 0})
    return fig


def monthly_returns_heatmap(equity_curve: pd.DataFrame) -> go.Figure | None:
