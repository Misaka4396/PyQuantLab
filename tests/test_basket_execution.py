...
def test_partial_fill_produces_unfilled():
    cfg = ExecutionConfig(
        ...
        partial_fill_ratio_max=0.6,
        delay_minutes_min=0.0,
        delay_minutes_max=0.0,
        seed=1,
    )
...
def test_delay_configurable():
    cfg = ExecutionConfig(
        delay_minutes_min=3.0, delay_minutes_max=3.0, partial_fill_prob=0.0, seed=0
    )
    executor = BasketExecutor(cfg, cost_model=_zero_cost_model())
...
def test_execution_cost_consistent_with_a3():
    cfg = ExecutionConfig(
        partial_fill_prob=0.0, delay_minutes_min=0.0, delay_minutes_max=0.0, seed=0
    )
    cost_cfg = CostConfig(
...
def test_creation_redemption_off_goes_secondary():
    cfg = ExecutionConfig(
        enable_creation_redemption=False,
    ...
    executor = BasketExecutor(cfg, cost_model=_zero_cost_model())
    res = executor.execute(
        [{"symbol": "510300", "side": "BUY", "quantity": 1000}],
        prices={"510300": 3.0},
    )
...
def test_creation_redemption_on_produces_cr():
    basket = _basket()
    cfg = ExecutionConfig(
        enable_creation_redemption=True,
        creation_unit=1000.0,
        creation_fee=10.0,
        partial_fill_prob=0.0,
        delay_minutes_min=0.0,
...
def test_make_basket_legs_direction():
    ...
    legs = make_basket_legs(
        ...
        etf_quantity=1000,
        etf_price=3.0,
        stock_prices={"A": 2.0, "B": 3.0},
        direction="long",
    )
    # long：ETF 腿 BUY，成分股腿 SELL
    etf_leg = next(leg for leg in legs if leg["symbol"] == "510300")
    assert etf_leg["side"] == "BUY"
    stock_legs = [leg for leg in legs if leg["symbol"] != "510300"]
    assert all(leg["side"] == "SELL" for leg in stock_legs)
    # 篮子名义 1000*3=3000；A 权重 200/(200+300)=0.4 → qty = 3000*0.4/2 = 600
    a = next(leg for leg in stock_legs if leg["symbol"] == "A")
