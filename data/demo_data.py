...
# akshare 抓取
# ---------------------------------------------------------------------------
def fetch_etf_daily_akshare(code: str, start: str, end: str, retries: int = 2) -> Optional[pd.DataFrame]:
    ...
    for attempt in range(retries):
        try:
            raw = ak.fund_etf_hist_em(symbol=code, period="daily", start_date=start, end_date=end, adjust="")
...
def fetch_constituents_akshare(index_code: str, retries: int = 2) -> Optional[List[str]]:
...
def _synthetic_membership(
    ...
    for i, code in enumerate(codes):
        if i == 1:
            # 第 2 只：中途调出（演示调出）
            rows.append({
                sc.COL_SYMBOL: code, sc.COL_ENTRY_DATE: start, sc.COL_EXIT_DATE: mid,
                sc.COL_AS_OF: start, sc.COL_REASON: sc.REASON_REMOVE,
            })
        else:
            rows.append({
                sc.COL_SYMBOL: code, sc.COL_ENTRY_DATE: start, sc.COL_EXIT_DATE: pd.NaT,
                sc.COL_AS_OF: start, sc.COL_REASON: sc.REASON_INITIAL,
            })
    # 一只虚构退市股：历史存在但当前快照没有（幸存者偏差核心演示）
    rows.append({
        sc.COL_SYMBOL: DELISTED_SYMBOL, sc.COL_ENTRY_DATE: start, sc.COL_EXIT_DATE: mid,
        sc.COL_AS_OF: start, sc.COL_REASON: sc.REASON_DELIST,
    })
    # 一只中途调入
    rows.append({
        sc.COL_SYMBOL: "601318", sc.COL_ENTRY_DATE: mid, sc.COL_EXIT_DATE: pd.NaT,
        sc.COL_AS_OF: mid, sc.COL_REASON: sc.REASON_ADD,
    })
...
def make_demo_data(
    ...
    for i, code in enumerate(ETF_CODES):
        df = fetch_etf_daily_akshare(code, start.strftime("%Y%m%d"), end.strftime("%Y%m%d")) if use_akshare else None
