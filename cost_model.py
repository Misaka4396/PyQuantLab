...
from cost_config import CostConfig, IMPACT_SQRT, IMPACT_LINEAR
...
class CostModel:
    ...
    def compute(
        ...
        stamp_tax = 0.0 if (is_etf and cfg.etf_exempt_stamp_tax) or side == BUY else exec_value * cfg.stamp_tax_rate
        transfer_fee = 0.0 if (is_etf and cfg.etf_exempt_transfer_fee) else exec_value * cfg.transfer_fee_rate
        ...
        details = [
            CostDetailItem("佣金", commission, f"费率 {cfg.commission_rate:.4%}，最低 {cfg.min_commission:.2f} 元"),
            CostDetailItem("印花税", stamp_tax,
                           "卖出 {:.4%}".format(cfg.stamp_tax_rate) + ("（ETF 免）" if is_etf and cfg.etf_exempt_stamp_tax else "")),
            CostDetailItem("过户费", transfer_fee,
                           "双向 {:.4%}".format(cfg.transfer_fee_rate) + ("（ETF 免）" if is_etf and cfg.etf_exempt_transfer_fee else "")),
            CostDetailItem("价差成本(半价差)", spread_cost, f"半价差 {half_spread_rate:.4%}"),
            CostDetailItem("冲击成本", impact_cost,
...
def sensitivity_table(
    ...
    for v in values:
        ...
        model = CostModel(cfg)
        b = model.compute(side=side, quantity=quantity, price=price,
                          symbol="510300", is_etf=is_etf, volume=volume)
